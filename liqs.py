...
#put the path of your binance.csv below
filename = 'C:/.../liquidationdata/crypto-liquidation-datastream/binance.csv'
...
async def binance_liquidation(websocket_url, filename):
    async with connect(websocket_url) as websocket:
        while True:
            try:
                ...
                color_name = 'Green' if side == 'SELL' else 'Red'
                if usd_size > 3000:
                    liquidation_type = 'L LIQ' if side == 'SELL' else 'S LIQ'
                    symbol = symbol[:4]
                    output = f"{liquidation_type} {symbol} occured at {time_est} with size of {usd_size:,.0f} USD {color_name}"
